...
#'
#' @noRd
.build_query <- function(x, columns, filter, column_filter) {
    if (missing(x))
        stop(" 'x' is required")
    if (missing(columns))
...
#' @export
fetch_data <- function(x, columns, filter, column_filter) {
    con <- .dbconn(x)
    if (length(.dbname(x)))
        on.exit(dbDisconnect(con))
    res <- dbGetQuery(con, .build_query(x, columns = columns,
                                               filter = filter,
